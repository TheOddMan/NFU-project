...
def _main():
    annotation_path = 'annotationTxt/train_VIVA_Hand.txt'
    log_dir = 'logs/000/'
    ...
    if is_tiny_version:
        ...
        with tf.device('/cpu:0'):
            template_model = create_model(input_shape, anchors, num_classes,
                freeze_body=2, weights_path='weights/VIVA_Hand/ep003-loss35.500-val_loss35.866.h5')
    ...
    if False:
        ...
        history_logs = model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                ...
                epochs=40,
                initial_epoch=0,
                callbacks=[lr_shedule, checkpoint])
    ...
    if True:
        ...
        batch_size = 64
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        history_all_logs = model.fit_generator(
